...
export class VoidreportMangementComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.route.snapshot.paramMap.get(this.resourceKey)) {
      this.route.paramMap.subscribe((params) => {
        const param = params.get(this.resourceKey);
        this.resourceKeyValue = param ? JSON.parse(param) : {};
        ;
      });
    }
    this.createForm();
  ...
  transferData(event: dxDataGrid<any, any>) {
    const doc = new jsPDF('l', 'px', 'a4');
    exportDataGridToPdf({
      jsPDFDocument: doc,
      component: event,
      // Remove autoTableOptions or adjust according to documentation
    }).then(() => {
